// API settings
const API_KEY = '2969fce395394571bcf403371a21bb8a';
// No need for API_HOST anymore as we're using our proxy
...
const leaguePriorities = {
    ...
    BL1: 70, // Bundesliga
    default: 50 // Lower divisions (limited in free tier)
  };
  ...
  function filterMatches(matches) {
    return matches.filter(match => {
      ...
      console.log('Match filtering:', {
        ...
        isRecent,
        isUpcoming,
        willBeIncluded: isAllowedLeague && (isLive || isRecent || isUpcoming)
      });
  ...
  function displayLiveScores(matches) {
    ...
    Object.entries(groupedMatches).forEach(([date, dateMatches]) => {
        ...
        dateMatches.forEach(match => {
            ...
            card.querySelector('.home-team-name').textContent = match.homeTeam?.name ?? 'Unknown';
            card.querySelector('.away-team-name').textContent = match.awayTeam?.name ?? 'Unknown';
        ...
        liveScoresDiv.appendChild(dateContainer);
    });
  }
